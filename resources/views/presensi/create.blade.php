    ...
    <div class="relative w-full h-[calc(100vh-70px)] bg-slate-900 overflow-hidden font-inter">
        ...
        <div class="absolute top-0 left-0 right-0 z-40 p-4 flex items-center justify-between pointer-events-none">
            ...
            <!-- Notification/Profile (Placeholder) -->
             <div class="w-10 h-10 flex items-center justify-center bg-black/20 backdrop-blur-md rounded-full text-white pointer-events-auto">
        ...
        <div class="absolute inset-0 z-0">
             ...
             <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[60%] w-64 h-80 rounded-[3rem] border-2 border-white/30 box-content shadow-[0_0_100px_rgba(0,0,0,0.5)_inset]">
                 <!-- Corners -->
                 <div class="absolute top-0 left-0 w-12 h-12 border-t-4 border-l-4 border-emerald-400 rounded-tl-[2.5rem]"></div>
                 <div class="absolute top-0 right-0 w-12 h-12 border-t-4 border-r-4 border-emerald-400 rounded-tr-[2.5rem]"></div>
                 <div class="absolute bottom-0 left-0 w-12 h-12 border-b-4 border-l-4 border-emerald-400 rounded-bl-[2.5rem]"></div>
                 <div class="absolute bottom-0 right-0 w-12 h-12 border-b-4 border-r-4 border-emerald-400 rounded-br-[2.5rem]"></div>
                 
                 <!-- Scanning Line Animation -->
                 <div class="absolute top-0 left-0 right-0 h-1 bg-emerald-400/50 shadow-[0_0_20px_rgba(52,211,153,1)] animate-scan"></div>
             </div>

             <!-- Date & Time Floating Pills -->
            <div class="absolute top-20 left-1/2 -translate-x-1/2 flex gap-3 z-10">
                <div class="bg-black/30 backdrop-blur-md border border-white/10 px-3 py-1 rounded-full flex items-center gap-1.5">
                    <ion-icon name="calendar-outline" class="text-emerald-400 text-xs"></ion-icon>
                    <span class="text-white text-xs font-medium">{{ \Carbon\Carbon::now()->translatedFormat('d M Y') }}</span>
                </div>
                <div class="bg-black/30 backdrop-blur-md border border-white/10 px-3 py-1 rounded-full flex items-center gap-1.5">
                    <ion-icon name="time-outline" class="text-emerald-400 text-xs"></ion-icon>
                    <span class="text-white text-xs font-medium" id="live-clock">{{ date('H:i:s') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="absolute bottom-0 left-0 right-0 z-50 bg-white rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.3)] pb-20 pt-8 px-6 transition-transform duration-300 transform translate-y-0">
            
            <!-- Floating Location Capsule (Sitting on top edge) -->
            <div class="absolute -top-6 left-1/2 -translate-x-1/2 w-[90%] max-w-sm bg-slate-900 text-white p-1.5 pr-4 rounded-full shadow-xl flex items-center justify-between ring-4 ring-white/10 backdrop-blur-sm">
                 <div class="flex items-center gap-3 flex-1 min-w-0">
                    <div class="w-8 h-8 rounded-full bg-emerald-500 flex items-center justify-center shrink-0 shadow-lg shadow-emerald-500/30">
                     ...
                     <div class="flex-col flex min-w-0 pr-2">
                        ...
                        <span class="text-xs font-bold truncate leading-tight" id="location-display">{{ $general_setting->multi_lokasi ? ($item->nama_cabang ?? 'Pilih Lokasi') : $lokasi_kantor->nama_cabang }}</span>
                     </div>
                 </div>
                 ...
                 @if ($general_setting->multi_lokasi)
                    <div class="relative">
                        <ion-icon name="chevron-down-circle" class="text-slate-500 text-xl"></ion-icon>
                         <select name="cabang" id="cabang" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onchange="document.getElementById('location-display').innerText = this.options[this.selectedIndex].text">
            ...
            <div class="bg-slate-50 border border-slate-100 rounded-2xl p-4 mb-6 flex items-center justify-between">
                ...
                <div class="text-center">
                    ...
                    <div class="flex items-center gap-1 text-slate-700">
                        <span class="text-sm font-bold font-mono">{{ date('H:i', strtotime($jam_kerja->jam_masuk)) }}</span>
                        <span class="text-slate-300">-</span>
                        <span class="text-sm font-bold font-mono">{{ $jam_kerja->jam_pulang ? date('H:i', strtotime($jam_kerja->jam_pulang)) : 'Selesai' }}</span>
                    </div>
                </div>
            </div>
