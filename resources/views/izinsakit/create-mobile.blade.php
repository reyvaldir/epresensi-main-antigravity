...
@section('content')
    ...
    <div class="flex items-center justify-between mb-5 mt-2">
        <div class="flex items-center gap-3">
            ...
            <h1 class="text-xl font-bold text-slate-800">Buat Izin Sakit</h1>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 mb-6">
        <form action="{{ route('izinsakit.store') }}" method="POST" id="formIzin" autocomplete="off"
            ...
            <!-- File Upload (SID) -->
            <div class="space-y-1">
                <label class="text-xs font-semibold text-slate-500 ml-1">Surat Dokter (Lampiran)</label>
                <div class="relative group">
                    <div
                        class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400 group-focus-within:text-primary transition-colors">
                        <ion-icon name="cloud-upload-outline" class="text-lg"></ion-icon>
                    </div>
                    <input type="file" name="sid" id="sid"
                        class="sid w-full pl-10 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20 transition-all focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                </div>
                <p class="text-[10px] text-slate-400 ml-1">*Upload surat keterangan dokter jika ada</p>
            </div>
            ...
            <div class="space-y-1">
                ...
                <div class="relative group">
                    ...
                    <textarea name="keterangan"
                        ...
                        placeholder="Jelaskan kondisi sakit..."></textarea>
                </div>
            </div>
    ...
    <script>
        ...
        $("#formIzin").submit(function (e) {
            ...
            if (dari == "" || sampai == "") {
                ...
                return false;
            } else if (parseInt(jml_hari) > 3) { // Legacy rule: Max 3 days for Izin Sakit via app?
                Swal.fire({
                    title: "Oops!",
                    text: 'Pengajuan Izin Sakit lewat aplikasi maksimal 3 hari. Lebih dari itu, silakan hubungi HRD.',
                    icon: "warning",
