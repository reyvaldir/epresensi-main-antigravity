<form action="{{ route('karyawan.update', Crypt::encrypt($karyawan->nik)) }}" id="formcreateKaryawan" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <x-input-with-icon-label icon="ti ti-barcode" label="NIK (Nomor Identitas Karyawan)" name="nik_show" value="{{ $karyawan->nik_show ?? $karyawan->nik }}" />
    ...
    <label style="font-weight: 600" class="form-label">Upload Foto Karyawan</label>
    <x-input-file label="Foto" name="foto"/>
